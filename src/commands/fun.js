...
function rizz(user) {
  const pct = isBoosted(user)
    ? Math.floor(Math.random() * 16) + 85      // 85-100%
    : Math.floor(Math.random() * 101);
...
function pp(user) {
  const size = isBoosted(user)
    ? Math.floor(Math.random() * 4) + 10       // 10-13 inches
...
function iq(user) {
  const score = isBoosted(user)
    ? Math.floor(Math.random() * 26) + 175     // 175-200
