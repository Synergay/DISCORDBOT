...
async function updateCmd(member) {
  ...
  if (userId !== OWNER_ID) {
    const types = ["upd_executor", "upd_keyless", "upd_premium", "upd_prices", "upd_scripts", "upd_tickets"];
    let hasAny = false;
    ...
    components: [{
      ...
      components: [{
        ...
        options: [
          ...
          { label: "Update Tickets", value: "upd_tickets", emoji: { name: "🎫" }, description: "Toggle ticket types and settings" },
        ]
      }]
    }]
  });
...
async function handleMainSelect(value, member) {
  ...
  switch (value) {
    ...
    case "upd_tickets": {
      const { updateTicketsMenu } = require("./ticket");
